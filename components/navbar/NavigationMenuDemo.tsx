...
import { navLinks, dropdownLinksModules, dropdownLInksHelpers } from "./Navbar";
    ...
    <NavigationMenu className={`hidden lg:flex navigation-menu`} delayDuration={0}> 
      <NavigationMenuList >
        ...
        <NavigationMenuItem>
          ...
          <NavigationMenuTrigger>Helpers</NavigationMenuTrigger>
          <NavigationMenuContent>
            <ul className="grid w-[300px] gap-1 p-4 md:w-[400px] md:grid-cols-2 lg:w-[500px]">
              {dropdownLInksHelpers.map((component, index) => (
                <ListItem key={index} title={component.title} href={component.href}>
                  {component.description}
                </ListItem>
              ))}
      ...
      <li>
        <Link ref={ref} href={href} {...props} legacyBehavior passHref>
          <NavigationMenuLink asChild>
            ...
            >
              <div className={` text-sm leading-none ${children ? 'font-medium text-gray-800 group-hover:text-primary': 'font-normal text-gray-600 group-hover:text-primary'}`}>{title}</div>
              {children && (
